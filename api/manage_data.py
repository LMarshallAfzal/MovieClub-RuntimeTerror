...
def change_rating_data(rating, new_score):
    local_ratings = open(file_path,'r')
    reader = csv.reader(local_ratings)
    lines = list()
    for row in reader:
        lines.append(row)
        if (row[0] == str(rating.user.id)) and (row[1] == str(rating.movie.movieID)):
            lines.remove(row)
    local_ratings.close()
    new_rating = [rating.user.id,rating.movie.movieID,new_score]
    lines.append(new_rating)
